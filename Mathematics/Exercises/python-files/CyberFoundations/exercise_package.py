...
class CoprimesError(Exception):
    def __init__(self, m_list):
        super().__init__(f" The integers in {m_list} are not pairwise coprimes. Pairwise coprimes required.")

class PositiveNumberError(Exception):
    def __init__(self, positive_number):
        super().__init__(f"Number {positive_number} is not positive. A positive number required.")
...
class SafePrimesError(Exception):
    def __init__(self, p):
        super().__init__(f"Number {p} is not a safe prime. A safe prime (p = 2q + 1), where q is prime, is required.")

class IterationLimitReached(Exception):
    def __init__(self, n, max_iteration):
        super().__init__(f"Iteration limit {max_iteration} exceeded. Failed to generate RSA keys.")
...
def is_primitive(g, p):
        ...
        """
        q = (p-1)//2
        if mod_pow_2(g, 1, p) == 0 or mod_pow_2(g, 1, p) == 1\
            or mod_pow_2(g, 1, p) == -1 or mod_pow_2(g, q, p) == 1:
            return False
        return True
...
def mod_inv(a: int, n: int) -> int:
    ...
    gcd, u, v = extended_gcd(a, n) # u*a + v*n = gcd
...
def is_prime(n: int, N: int = 40)-> tuple[bool, list]:
    """
